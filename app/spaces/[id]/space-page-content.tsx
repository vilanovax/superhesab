...
import { resolveDefaultTab } from "@/lib/spaces/space-tab-data";
...
type SpaceMembership = NonNullable<
...
export async function SpacePageContent({
  ...
  /**
   * Critical path: chrome + hero + active tab only.
   * Report / debts / funds / checklist / chargeProofs load via deferred tab.
   */
  const [space, balanceData, monthRows, buildingView, openBoardSuggestions, fundDashboard, fundProofs, deferredTab] =
    await Promise.all([
      ...
      // Settlements chrome (share summary) needs balances on every open.
      ...
      features.incomeExpense || features.budget
        ...
        : Promise.resolve([]),
      features.buildingCharges
        ? getBuildingManagerView(id, planYear)
  ...
  return (
    ...
    >
      ...
      <SpaceTabsGate
        ...
        tabLoadContext={{
          planYear: features.buildingCharges ? planYear : undefined,
