# سند نیازمندی‌های محصول (PRD) - تمپلیت خانه (`FAMILY`)

> **ادغام ۱۴۰۵:** قالب شخصی (`PERSONAL`) در فاز ۱ در همین قالب ادغام شد.  
> سند ادغام: [`home-ledger-merge-prd.md`](./home-ledger-merge-prd.md). لیبل UI: «خانه».

## ۱. هویت و هدف محصول (Product Vision)

یک کیف پول برای مدیریت اقتصاد شخصی یا خانواده. این تمپلیت پیچیدگی‌های دنگ‌ودونگ و تسویه‌حساب (Settlement) را به طور کامل حذف می‌کند تا فرد یا اعضای خانه بتوانند درآمدها و هزینه‌های روزمره را یکپارچه ثبت کنند. تک‌نفره = شخصی سابق؛ با دعوت اعضا همان لجر اشتراکی می‌شود.

*نکته محصولی: این تمپلیت کاملاً از تمپلیت `PARTNER` متمایز است. `PARTNER` برای هزینه‌های دونفره با قابلیت تسویه است، در حالی که خانه (`FAMILY`) لجر **بدون تسویه بین اعضا** است. ماژول جداگانهٔ بدهی/طلب بیرونی (`debts`) مجاز است و با Settlement مخلوط نمی‌شود.*
